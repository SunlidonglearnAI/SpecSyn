...
def evaluate(args):
    ...
    try:
        ...
        # ===== 隐藏装饰场景网格 =====
        for i in range(raw_env.sim.model.ngeom):
            geom_type = raw_env.sim.model.geom_type[i]
            contype   = raw_env.sim.model.geom_contype[i]
            # mesh类型(7) 且无碰撞的geom全部透明化（装饰物）
            if geom_type == 7 and contype == 0:
                raw_env.sim.model.geom_rgba[i][3] = 0.0
                name = raw_env.sim.model.geom_id2name(i)
                print(f"DEBUG: Hidden decorative geom '{name}'")
    except Exception as e:
