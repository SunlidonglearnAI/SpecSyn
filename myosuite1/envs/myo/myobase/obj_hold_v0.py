...
class ObjHoldFixedEnvV0(BaseV0):
    ...
    # === 修改 1: 调整权重 ===
    DEFAULT_RWD_KEYS_AND_WEIGHTS = {
        "goal_dist": 20.0,   # 原来是 100.0 (太大了)，降到 20.0
        "bonus": 4.0,        # 保持不变 (接近目标的额外奖励)
        "penalty": 50.0,     # 原来是 10.0，增加掉落惩罚，防止它摆烂扔球
        "alive": 2.0,        # [新增] 生存奖励！只要拿着就有分
    }
    ...
    def get_reward_dict(self, obs_dict):
        # 1. 计算基础数值
        goal_dist = np.abs(np.linalg.norm(self.obs_dict["obj_err"], axis=-1))
        act_mag = (
            np.linalg.norm(self.obs_dict["act"], axis=-1) / self.sim.model.na
            if self.sim.model.na != 0
            else 0
        ...
        # 2. 构造奖励字典 (所有 Value 必须被 float() 包裹)
        rwd_dict = collections.OrderedDict(
            (
                # Optional Keys
                ("goal_dist", -1.0 * float(goal_dist)),
                # === 修复点：bonus 必须强转 float ===
                (
                    "bonus",
                    float(1.0 * (goal_dist < 2 * gaol_th) + 1.0 * (goal_dist < gaol_th)),
                ),
                ("act_reg", -1.0 * float(act_mag)),
                ("penalty", -1.0 * float(drop)),
                # === 修复点：alive 使用 (1-drop) 并强转 float ===
                ("alive", 1.0 * (1.0 - float(drop))),
                
                # Must keys
                ("sparse", -float(goal_dist)),
                ("solved", goal_dist < gaol_th),
                ("done", drop),
            )
        )
        ...
        # 3. 加权求和
        # 此时 list 里全是纯 float，np.sum 绝对不会报错
        rwd_dict["dense"] = np.sum(
            [wt * rwd_dict[key] for key, wt in self.rwd_keys_wt.items()], axis=0
        )
        return rwd_dict
