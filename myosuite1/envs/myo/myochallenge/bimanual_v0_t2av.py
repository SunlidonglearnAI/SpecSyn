...
class T2ABimanualVelocityMixin:
    """
    Mixin for Evolution of Muscle Velocity (Vmax) only.
    """
    def _setup_t2a(self, **kwargs):
        ...
        print(f"DEBUG: T2A Bimanual Setup - Mode: Velocity (Vmax) Only. Dim: {self.design_dim}")
        return kwargs
    ...
    def _apply_design(self, design_act):
        ...
        Action range [-1, 1] -> Velocity [0.5, 1.5]
        """
        ...
        # 映射公式: -1 -> 0.5 (变慢), 0 -> 1.0 (不变), 1 -> 1.5 (变快)
        v_scale = 1.0 + 0.5 * design_params
        
        # 修改 MuJoCo 模型中的 Vmax (gainprm 索引为 6)
    ...
    def visualize_evolution(self, save_path="bimanual_velocity_evolution.png", show=False):
        if self.current_scales is None: return
        plt.figure(figsize=(6, 12)) 
        sns.heatmap(self.current_scales, 
                    yticklabels=self.muscle_names, 
                    xticklabels=["Velocity (Vmax)"],
                    cmap="coolwarm", center=1.0, annot=True, fmt=".2f") 
        plt.title("Muscle Velocity Evolution")
        plt.tight_layout()
        plt.savefig(save_path, dpi=300)
        if show: plt.show()
        plt.close()

class BimanualEnvT2A(T2ABimanualVelocityMixin, BimanualEnvV1):
