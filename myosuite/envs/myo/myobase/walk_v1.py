...
class WalkEnvV0(BaseV0):
    ...
    def get_reward_dict(self, obs_dict):
        vel_reward = float(self._get_vel_reward()) # 强制标量
        cyclic_hip = float(self._get_cyclic_rew()) # 强制标量
        ref_rot = float(self._get_ref_rotation_rew()) # 强制标量
        joint_angle_rew = float(self._get_joint_angle_rew(
            ["hip_adduction_l", "hip_adduction_r", "hip_rotation_l", "hip_rotation_r"]
        ))
        
        vel = self._get_com_velocity()
        alive_bonus = 1.0 if float(vel[1]) > 0.1 else 0.0

        act_mag = (
            float(np.linalg.norm(self.obs_dict["act"], axis=-1) / self.sim.model.na)
            if self.sim.model.na != 0
            else 0.0
        )
    ...
    def _get_vel_reward(self):
        ...
        """
        vel = self._get_com_velocity()
        x_vel_err = self.target_x_vel - vel[0]
        
        # 只要往前走就有正收益，站着不动就是 0
        return vel[1] - 0.5 * np.abs(x_vel_err)
...
class TerrainEnvV0(WalkEnvV0):
    ...
    def reset(self, **kwargs):
        self.steps = 0
        # # 获取 XML 中定义的实际行列数，不要写死 10000
        # nrow = self.sim.model.hfield_nrow[0]
        # ncol = self.sim.model.hfield_ncol[0]
        # total_points = nrow * ncol # 比如 700*700 = 490000
